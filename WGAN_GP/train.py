...
from utils import weights_init, get_gradient_penalty, disc_loss, gen_loss
import numpy as np
...
n_critic = 5
# Grad Penalty Lambda Value for Discriminator Loss
c_lambda = 10
# Monitor Progress
progress = list()
...
for epoch in range(num_epochs):
    for i, data in enumerate(dataloader, 0):
        ...
        for _ in range(n_critic):
            ...
            epsilon = torch.rand(num_images, 1, 1, 1, device=device, requires_grad=True)
            grad_pen = get_gradient_penalty(disc_net, real_images, fake_images.detach(), epsilon)
            disc_err = disc_loss(real_output, fake_output, grad_pen, c_lambda)
